...
url = "https://n582759.yclients.com/api/v1/book_staff/550726?datetime=&without_seances=1"
...
cookies = {
    ...
    'yc_utm_term': '',
    'yc_utm_click_id': '',
    'yc_utm_source': 'www.yandex.ru',
    'yc_utm_medium': 'referral',
    'yc_referral_url': 'https%3A%2F%2Fwww.yandex.ru%2F',
    'original_utm_referer_v2': '(not_set)',
    'tmr_lvid': 'd21edc0346d62ca0e788f4615df6402b',
    'tmr_lvidTS': '1719040219822',
    '_ym_uid': '1719040221369778136',
    '_ym_d': '1719040221',
    'landing_lang': 'ru',
    'analytics-udid': 'sNEZDjTgnWQh7a2urhD4R8bB0yEMPA1mmj6qNtGm',
    '_ga_4Z5R7DZBLZ': 'GS1.2.1719222722.2.0.1719222722.60.0.0',
    '_ga_CZ0CKD8R74': 'GS1.1.1719222720.3.0.1719222730.0.0.0',
    '_ga': 'GA1.1.2028114192.1719040219',
    '_cfuvid': 'sir0HVaGrZ3M08GsGbnfzdeE0FHhgXWgdonmv9S2bRA-1725276900979-0.0.1.1-604800000',
    'lang': '1',
    'ycl_language_id': '1',
    'erp_language_id': '1',
    '_gcl_au': '1.1.124876351.1730372747',
    'original_utm_source_v2': 'www.yandex.ru',
    'original_utm_medium_v2': 'organic',
    'original_utm_campaign_v2': '(not_set)',
    'original_utm_term_v2': '(not_set)',
    '_ga_P2LM7D8KSM': 'GS1.1.1730372748.6.1.1730372950.60.0.0',
    '_ga_X3P164PV59': 'GS1.1.1730372748.3.1.1730372950.60.0.0',
    'spid': '1730373076799_c9d56d327325e5eee42fa3a4a0ffd98c_lvuepjluwlhg3vpr',
    'spsc': '1730373207657_97c63346c230fe2593533a6d26d3f958_e6cfb3ea8f0a0fa28cc6ebefdcae8ea5',
    'app_service_group': '0',
    '_ym_isad': '2',
    '_ym_visorc': 'b',
    '__ga_track__': 'GS1.1-2.1664391493.1.1.1664391604.0.0.0.OUdUaDFNN1k0TXVQMXRTWG5xT2lxS3VtcXJCNms1N0o4QT09',
    'tracking-index': '859',
}
...
def pars_services_mast(master_id: str) -> dict:
    url = f"https://n582759.yclients.com/api/v1/booking/locations/550726/search/services?staff_id={master_id}"
...
def pars_services() -> dict:
    url = "https://n582759.yclients.com/api/v1/book_services/550726"
    r = requests.get(url=url, headers=headers)
...
def pars_time(master_id: str, service_id: str, date: str) -> dict:
    url = f"https://n582759.yclients.com/api/v1/book_times/550726/{master_id}/{date}?&service_ids%5B%5D={service_id}"
...
def pars_date(master_id: str, service_id: str) -> dict:
    url = f"https://n582759.yclients.com/api/v1/book_dates/550726?staff_id={master_id}&date=&date_from=2024-07-01&date_to=9999-01-01&service_ids%5B%5D={service_id}"
    r = requests.get(url=url, headers=headers)
...
def to_booking(json: dict):
    r = requests.post(url="https://api.yclients.com/api/v1/book_record/550726", json=json, headers=headers, cookies=cookies)
    print(r.text)
...
async def get_user_to_id(id: int):
    async with pg_manager:
        user = await pg_manager.select_data(table_name='users_reg', where_dict={'user_id': id})
    if user:
        return user[0]
